...
export default function ManageArticles() {
  ...
  const buildTree = (flatArticles: Article[]): Article[] => {
    const idToNodeMap = new Map<number, Article & { children?: Article[] }>();
    const roots: (Article & { children?: Article[] })[] = [];

    flatArticles.forEach(article => {
      idToNodeMap.set(article.id, { ...article, children: [] });
    });

    flatArticles.forEach(article => {
      const node = idToNodeMap.get(article.id)!;
      if (article.parentId != null) {
        const parent = idToNodeMap.get(article.parentId);
        if (parent) {
              ...
              <CardContent>
                {Array.isArray(articles) && articles.length > 0 ? (
                  <div className="space-y-2">
                    {renderArticleTree(buildTree(articles))}
                  </div>
                ) : (
